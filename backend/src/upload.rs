...
pub async fn upload_thumbnail_chunk(
    ...
    if completed {
        ...
        return Ok(Json(ApiResponse::success(ChunkUploadResponse {
            ...
            next_chunk: None,
            completed: true,
            file_path: Some(PathBuf::from("thumbnails").join(&new_filename).to_string_lossy().to_string()),
...
pub async fn upload_channel_image_chunk(
    ...
    if completed {
        ...
        return Ok(Json(ApiResponse::success(ChunkUploadResponse {
            ...
            next_chunk: None,
            completed: true,
            file_path: Some(PathBuf::from("channels").join(&new_filename).to_string_lossy().to_string()),
            disk_location: Some("local".to_string()),
            file_size: Some(info.total_size),
            disk_id: None,
        })));
